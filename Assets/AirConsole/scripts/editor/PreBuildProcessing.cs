...
namespace NDream.AirConsole.Editor {
    public class PreBuildProcessing : IPreprocessBuildWithReport {
            ...
            if (File.Exists(manifestPath)) {
                ...
                foreach (XElement activityElement in activityElements) {
                    ...
                    if (activityName == UNITY6_ANDROID_ACTIVITY_NAME || activityName == UNITY_ANDROID_ACTIVITY_NAME) {
                        UpdateManifestActivityEntry(activityElement, name, themeAttribute);
                    }
                }
        ...
        private static void UpdateManifestActivityEntry(XElement activityElement, XAttribute name, XName themeAttribute) {
            if (Application.unityVersion.Contains("6000")
                && (GetAndroidApplicationEntry() & AndroidActivityEntry.GameActivity) == AndroidActivityEntry.GameActivity) {
        ...
        private static AndroidActivityEntry GetAndroidApplicationEntry() {
            SerializedObject playerSettings = new(AssetDatabase.LoadAllAssetsAtPath("ProjectSettings/ProjectSettings.asset")[0]);
            SerializedProperty androidApplicationEntry = playerSettings.FindProperty("androidApplicationEntry");
            return (AndroidActivityEntry)androidApplicationEntry.intValue;
        }

        [Flags]
        private enum AndroidActivityEntry {
            Activity = 1,
            GameActivity = 2
        }
