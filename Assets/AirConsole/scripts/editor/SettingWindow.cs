...
namespace NDream.AirConsole.Editor {
	public class SettingWindow : EditorWindow {
		...
		private const string AC_LATEST_VERSION_PREFS = "AC_LATEST_VERSION";
		private readonly GUIStyle styleBlack = new GUIStyle();
		private readonly GUIStyle updateBanner = new GUIStyle();
		private readonly GUIStyle styleRedBold = new GUIStyle();
		private readonly Color darkRed = new Color(0.7f, 0.0f, 0.0f);
		...
		private static void Init() {
			if(!EditorPrefs.HasKey(AC_LATEST_VERSION_PREFS)) {
				HttpWebRequest request =
					(HttpWebRequest)WebRequest.Create("https://api.github.com/repos/AirConsole/airconsole-unity-plugin/releases");
				request.Method = "GET";
				...
				try {
					using (HttpWebResponse response = (HttpWebResponse)request.GetResponse()) {
						...
						if((int)response.StatusCode == 200) {
							...
							List<float> versions = releases?.Where(t => t.HasValues && !string.IsNullOrEmpty(t["tag_name"].ToString()))
							                               .Select(t => t["tag_name"].ToString().Replace("v", ""))
							                               .Select(float.Parse)
							                               .ToList();
							if(versions != null) EditorPrefs.SetFloat(AC_LATEST_VERSION_PREFS, versions.First());
						}
					}
				}
				catch (Exception e) {
		...
		private void OnGUI() {
			...
			ApplyDefaultWebGLSettings();
			ApplyAndroidRequiredSettings();
		}

		private void DrawUpdatedVersionNotification() {
			if(EditorPrefs.HasKey(AC_LATEST_VERSION_PREFS) && float.TryParse(Settings.VERSION, out float pluginVersion) &&
			   EditorPrefs.GetFloat(AC_LATEST_VERSION_PREFS, 0f) > pluginVersion) {
				...
				GUILayout.FlexibleSpace();
				if(GUILayout.Button("Download now")) Application.OpenURL("https://github.com/AirConsole/airconsole-unity-plugin/releases");
				EditorGUILayout.EndHorizontal();
				EditorGUILayout.Space(20);
			}
		}
		...
		private void DrawFooter() {
			...
			GUILayout.FlexibleSpace();
			if(GUILayout.Button("Reset Settings", GUILayout.MaxWidth(110))) Extentions.ResetDefaultValues();
		...
		private void DrawEditorSettings() {
			...
			if(GUILayout.Button(new GUIContent("Force Reset", "Will stop existing Unity WebSocket Servers on this port"),
			...
			if(newWebserverPort != Settings.webServerPort) {
				if(Extentions.webserver.IsRunning() && !Extentions.webserver.IsBlocked) Extentions.webserver.Stop();
			...
			if(Extentions.webserver.IsBlocked)
				EditorGUILayout
					.LabelField($"Webserver Port {Settings.webServerPort} already in use.\nAirConsole is not able to communicate with the Simulator",
					            styleRedBold);
			else {
				...
				GUILayout.Space(150);
				if(GUILayout.Button("Stop", GUILayout.MaxWidth(60))) Extentions.webserver.Stop();
				if(GUILayout.Button("Restart", GUILayout.MaxWidth(60))) Extentions.webserver.Restart();
		...
		private void DrawAndroidFoldout() {
			...
			if(PlayerSettings.Android.blitType != AndroidBlitType.Auto && PlayerSettings.Android.blitType != AndroidBlitType.Never)
				GUILayout.Label("Unless your tests on AndroidTV hardware requires this, we recommended to use \"Never\" or \"Auto\"",
				                styleRedBold);
			...
			if(currentLevel != newLevel) PlayerSettings.SetManagedStrippingLevel(BuildTargetGroup.Android, newLevel);
			EditorGUILayout.EndHorizontal();
			...
			if(!PlayerSettings.GetMobileMTRendering(BuildTargetGroup.Android)) {
				if(!EditorUtility.DisplayDialog("Verification",
				                                "You do currently not have Multithreaded Rendering enabled.\nPlease confirm that this is required as it decreases performance",
				                                "Yes", "No")) {
			...
			if(!PlayerSettingsHelper.GetIsLowLightmapEncodingQualityForPlatformGroup(BuildTargetGroup.Android)) {
				if(!EditorUtility.DisplayDialog("Verification",
				                                "You do currently not have Low Lightmap Encoding Quality enabled for Android.\nPlease confirm that you do not use Lightmaps in ES2 otherwise you will face issues",
				                                "Yes", "No")) {
			...
			if(newApiLevel != ApiCompatibilityLevel.NET_Standard_2_0)
				GUILayout.Label("Unless your tests on AndroidTV show a negative impact it is recommended to use .NET Standard as it increases performance and decreases the size.",
				                styleRedBold);
		...
		private void DrawCustomLauncherGradleWidget(bool requiresGradleExtension) {
			...
			if(disabledLauncherTemplateExists || launcherTemplateExists) {
				GUI.enabled = !Application.isPlaying && (requiresGradleExtension || launcherTemplateExists);
		...
		private void DrawCustomMainGradleWidget(bool requiresGradleExtension) {
			...
			if(disabledMainTemplateExists || mainTemplateExists) {
				GUI.enabled = !Application.isPlaying && (requiresGradleExtension || mainTemplateExists);
		...
		private void DrawGeneralSettings() {
			...
			if(!PlayerSettings.gcIncremental) GUILayout.Label("We recommended to enable this for the best game experience", styleRedBold);
		}
		...
		private void DrawWebGLFoldout() {
			...
			EditorGUILayout.EndHorizontal();
			if(newApiLevel != ApiCompatibilityLevel.NET_Standard_2_0)
				GUILayout.Label("Unless your tests on WebGL show a negative impact it is recommended to use .NET Standard as it increases performance and decreases the size.",
				                styleRedBold);
		...
		internal static void ApplyAndroidRequiredSettings() {
			...
			bool isOpenGles2Enabled = androidGraphicsAPIs.Contains(GraphicsDeviceType.OpenGLES2) &&
		...
		internal static void CheckUnityVersionForBuildSupport() {
			if(!Application.unityVersion.Contains("202") && EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android &&
		...
		private static readonly List<string> packagesFound = new List<string>();
						...
						case StatusCode.Success: {
							foreach (PackageInfo packageInfo in Request.Result) {
								packages.Where(package => packageInfo.packageId.StartsWith($"{package}@"))
								        .ToList()
								        .ForEach(package => packagesFound.Add(package));
							}
		...
		internal static void LogFoundDisallowedPackages() {
			packagesFound.ForEach(it =>
				                      Debug.LogError($"AirConsole Android Error: Please remove package \"{it}\" from 'Window > Package Manager'"));
		}
		...
		internal static void QueryAndApplyRecommendedAndroidSettings() {
			if(PlayerSettings.GetManagedStrippingLevel(BuildTargetGroup.Android) != ManagedStrippingLevel.High) {
				if(!EditorUtility.DisplayDialog("Verification",
				                                "You do currently not have HIGH Code Stripping Level active. Please confirm that this is required",
				                                "Yes", "No")) {
			...
			if(PlayerSettings.GetApiCompatibilityLevel(BuildTargetGroup.Android) != ApiCompatibilityLevel.NET_Standard_2_0) {
				if(!EditorUtility.DisplayDialog("Verification",
				                                "You do currently not have API Compatibility Level set to.NET Standard 2.0.\nPlease confirm that this is required as it decreases performance and increases the size",
				                                "Yes", "No")) {
		...
		internal static void QueryAndApplyRecommendedWebGLSettings() {
			if(PlayerSettings.WebGL.compressionFormat != WebGLCompressionFormat.Gzip) {
				if(EditorUtility.DisplayDialog("Verification", "You do currently not use GZip Compression. This is", "a release",
				                               "a test build")) {
			...
			if(PlayerSettings.WebGL.exceptionSupport != WebGLExceptionSupport.None) {
				if(EditorUtility.DisplayDialog("Verification", "You do currently not have WebGL exception support disabled. This is",
				                               "a release", "a test build")) {
			...
			if(PlayerSettings.WebGL.compressionFormat == WebGLCompressionFormat.Brotli)
				if(!EditorUtility.DisplayDialog("Error", "AirConsole does not currently support Brotli compression for WebGL builds",
				                                "Cancel"))
		...
		private static void ApplyDefaultWebGLSettings() {
			if(PlayerSettings.WebGL.template.Equals("APPLICATION:Default") || PlayerSettings.WebGL.template.Equals("APPLICATION:Minimal")) {
				...
				PlayerSettings.WebGL.template = Application.unityVersion.Substring(0, 3) == "202" ? "APPLICATION:AirConsole-2020"
					: "APPLICATION:AirConsole";
				EditorUtility.DisplayDialog("Information",
				                            $"Updated WebGL Template from {originalTemplate.Replace("APPLICATION", "")} to {PlayerSettings.WebGL.template.Replace("APPLICATION:", "")}",
				                            "OK");
			}
		}
