namespace NDream.AirConsole.Examples {
    ...
    public class ExampleBasicLogic : MonoBehaviour {
        ...
        private void OnCustomDeviceStateChange(int device_id, JToken custom_data) {
            //Log to on-screen Console
            logWindow.text = logWindow.text.Insert(0,
                "Custom Device State Change on device: " + device_id + ", data: " + custom_data + "\n \n");
        }

        private void OnDeviceProfileChange(int device_id) {
            //Log to on-screen Console
        ...
        private void OnHighScoreStored(JToken highscore) {
            //Log to on-screen Console
            if (highscore == null) {
        ...
        public void StoreHighScore() {
            JObject testData = new();
            testData.Add("test", "data");
            AirConsole.instance.StoreHighScore("Basic Example", "v1.0", highScore,
                AirConsole.instance.GetUID(AirConsole.instance.GetMasterControllerDeviceId()), testData);
        }

        public void StoreTeamHighScore() {
            List<string> connectedUids = new();
        ...
        public void StorePersistentData() {
            ...
            JObject testData = new();
            testData.Add("test", "data");
            AirConsole.instance.StorePersistentData("custom_data", testData,
                AirConsole.instance.GetUID(AirConsole.instance.GetMasterControllerDeviceId()));
        }
